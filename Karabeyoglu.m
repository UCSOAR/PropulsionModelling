function r_dot = Karabeyoglu(G, z)
    ...
    mu_g = 7.376E-5;%Pa-s  %Average gaseous viscosity in port -> guess
   ...
   %G = 400;%kg/m^2-s  %Oxidizer mass flux -> sourced from Dracarys
   %z = 0.01;%m  %Position along fuel grain -> variable parameter
    ...
    a_ent = 8E-12;  %Entrainment coefficient -> guess based on Development and testing of Paraffin-based Hybrid Rocket Fuels
    ...
    while converged == false
        ...
        if converge < 0.001 && (converge_change < .5 || isnan(converge_change)) 
            converged = true;
